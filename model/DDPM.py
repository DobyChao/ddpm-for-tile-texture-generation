...
class DDPM(nn.Module):
    ...
    def sample(self, n_sample, size, notqdm=False, guide_w=0.0, n_classes=10, selected=None):
        sche = self.ddpm_sche
        c, mask = self.prepare_condition_(n_sample, n_classes, selected)
    ...
    def ddim_sample(self, n_sample, size, steps=250, eta=1.0, notqdm=False, guide_w=0.0, n_classes=10, selected=None):
        sche = self.ddim_sche
        c, mask = self.prepare_condition_(n_sample, n_classes, selected)
        x_i = torch.randn(n_sample, *size).to(self.device)
    ...
    def prepare_condition_(self, n_sample, n_classes, selected=None):
        if selected is not None:
            c = torch.tensor(selected).to(torch.int64).to(self.device)
        else:
